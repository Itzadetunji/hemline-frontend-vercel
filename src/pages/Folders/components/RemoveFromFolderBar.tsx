...
interface RemoveFromFolderBarProps {
  folderId: string;
  folderName?: string;
}

export const RemoveFromFolderBar = ({ folderId, folderName }: RemoveFromFolderBarProps) => {
  ...
  return (
    <>
      <ul class="-translate-x-1/2 fixed bottom-6 left-1/2 flex items-center gap-0 border border-line-500">
        <Button class="gap-1 px-4 py-2.5 text-black capitalize" variant="secondary" onClick={handleRemoveFromFolder}>
          <div class="min-w-4.5">
            <Icon icon="material-symbols:remove-rounded" className="size-4.5" />
          </div>
          <p class="leading-0">Remove from Folder</p>
        </Button>
      ...
      <RemoveFromFolder
        ...
        image_ids={selectingSignal.value.selectedItems}
