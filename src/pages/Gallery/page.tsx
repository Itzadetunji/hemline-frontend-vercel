...
import { Dispatch, StateUpdater, useEffect, useLayoutEffect, useRef, useState } from "preact/hooks";
...
const GalleryImage = (props: {
    ...
    return (
        ...
        >
            {selectingImagesSignal.value.isSelecting ? (
                <label class="absolute top-1.5 left-1.5 cursor-pointer group">
                    ...
                    <div class="border size-4.5 flex items-center justify-center transition-colors border-line-500 bg-white/40 group-has-[:checked]:border-primary group-has-[:checked]:bg-primary">
                        <Icon icon="heroicons:check-20-solid" className="text-white h-3.5 w-3.5 opacity-0 transition-opacity group-has-[:checked]:opacity-100" />
                    </div>
                </label>
            ) : (
            ...
            <img src={cachedUrl} alt={props.image.file_name} onLoad={handleImageLoad} class={cn("w-full h-full object-cover transition-opacity duration-300", loading ? "opacity-0" : "opacity-100")} crossOrigin="anonymous" />
        </div>
    );
};
...
const EmptyGallery = () => (
    <div class="flex flex-col flex-1 justify-center items-center gap-4">
        <div class="flex flex-col gap-4 items-center">
            ...
            <p class="text-grey-500 font-medium text-sm">Upload your best works from your gallery</p>
        </div>
