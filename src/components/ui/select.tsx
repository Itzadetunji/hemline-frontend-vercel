...
export const Select = ({ options, value, onChange, placeholder = "Select...", icon = "fluent:chevron-down-24-regular", maxItems = Number.POSITIVE_INFINITY, className, disabled = false }: SelectProps) => {
  ...
  return (
    <div ref={containerRef} class={cn("relative w-full", className)}>
      ...
      {/* Dropdown */}
      {isOpen && (
        <div ref={dropdownRef} class="absolute z-50 mt-1 max-h-60 w-full overflow-y-auto border border-line-500 bg-white p-2 shadow-sm dark:bg-dark" style={{ top: "calc(100% + 4px)" }}>
          {options.length === 0 ? (
                ...
                return (
                  <li key={option.value}>
                    ...
                    >
                      ...
                      <input type="checkbox" checked={isSelected} onChange={() => handleOptionToggle(option.value)} disabled={isDisabled} class="size-4 cursor-pointer accent-primary" />
                    </label>
                  </li>
                );
              })}
