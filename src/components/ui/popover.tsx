...
export function Popover({ children, defaultOpen = false, open: controlledOpen, onOpenChange }: PopoverProps) {
  ...
  return <PopoverContext.Provider value={{ open, setOpen, triggerRef, contentRef, zIndex }}>{children}</PopoverContext.Provider>;
}
...
export function PopoverTrigger({ asChild, children, className, ...props }: PopoverTriggerProps) {
  ...
  return (
    <button ref={triggerRef} type="button" onClick={handleClick} aria-expanded={open} aria-haspopup="dialog" className={className} {...props}>
      {children}
    </button>
  );
}
...
export function PopoverContent({ children, className, align = "center", side = "bottom", sideOffset = 8, alignOffset = 0, ...props }: PopoverContentProps) {
