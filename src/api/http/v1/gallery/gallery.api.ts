...
export const GALLERY_API = {
	...
	GET_GALLERY_IMAGE: async (id: string): Promise<GetGalleryImageResponse> =>
		await $http
	...
	UPDATE_GALLERY_IMAGE: async (
		id: string,
		data: UpdateGalleryImagePayload
	): Promise<UpdateGalleryImageResponse> =>
		await $http
			.patch(GALLERY_ENDPOINTS.updateGalleryImage(id), data)
			.then((res) => res.data),

	DELETE_IMAGES: async (
		data: DeleteImagesPayload
	): Promise<DeleteImagesResponse> =>
		await $http
			.delete(GALLERY_ENDPOINTS.deleteImages, { data })
			.then((res) => res.data),
};
