...
export const Header = () => {
  ...
  const userData = (getUserProfile.data?.data as NotMarkedForDeletionProfile)?.user ?? userSignal.value?.user;
  const displayUser = userData ?? ({} as NotMarkedForDeletionProfile["user"]);

  const { cachedUrl, isLoading: loading } = useImageCache(displayUser.business_image ?? "");
          ...
          <Popover>
              ...
              <li class="relative size-9 overflow-hidden rounded-full">
                {displayUser.business_image ? (
                  ...
                  <div class="grid size-9 place-content-center rounded-full bg-primary text-center font-medium text-white text-xl">
                    {getInitials(displayUser.business_name ?? displayUser.full_name ?? "")}
                  </div>
                )}
              </li>
            </PopoverTrigger>
            <PopoverContent className="!top-14 !right-124 mr-4 flex w-40 flex-col items-stretch rounded-sm border-line-400 bg-white/70 drop-shadow-[0.6px_0.8px_9px_rgba(0,0,0,0,95)] backdrop-blur-lg">
              <ul class="flex flex-col gap-3">
                <li class="flex cursor-default items-center justify-between gap-2 px-2 py-1 text-grey-500">
                    <span class="font-medium text-sm">Status</span>
                    <span
                      class={cn(
                        "flex items-center gap-1 font-medium text-xs",
                        isOnlineSignal.value ? "text-green-600" : "text-amber-600"
                      )}
                    >
                      <Icon
                        icon={isOnlineSignal.value ? "hugeicons:connect" : "hugeicons:disconnect"}
                        className="size-4"
                      />
                      {isOnlineSignal.value ? "Online" : "Offline"}
                    </span>
                  </li>
